...
@Composable
fun BallZigZagDeflectIndicator(
        ...
        when (position) {
            in 0.0..1.0 -> {
                ...
                *  and the destination is the upper right vertex, which is named xTopRight. is called.
                 */
                x1 = (xTopRight + position * (xTopLeft - xTopRight))
                y1 = (yTopRight + position * (yTopLeft - yTopRight))

                /*
                * Calculate the coordinates of the circle that is on the lower triangle
                 */
                x2 = (xBottomLeft + position * (xBottomRight - xBottomLeft))
                y2 = (yBottomLeft + position * (yBottomRight - yBottomLeft))
            }

            in 1.0..2.0 -> {
                x1 = xTopLeft + (position - 1) * (xBottom - xTopLeft)
                y1 = yTopLeft + (position - 1) * (yBottom - xTopLeft)

                x2 = xBottomRight + (position - 1) * (xTop - xBottomRight)
                y2 = yBottomRight + (position - 1) * (yTop - yBottomRight)
            }

            in 2.0..3.0 -> {
                x1 = xBottom + (position - 2) * (xTopRight - xBottom)
                y1 = yBottom + (position - 2) * (yTopRight - yBottom)

                x2 = xTop + (position - 2) * (xBottomLeft - xTop)
                y2 = yTop + (position - 2) * (yBottomLeft - yTop)
            }
        }
